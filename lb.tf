...
resource "google_compute_backend_service" "backend" {
  ...
  port_name             = "tomcat" # MIG 구성의 포트 이름과 동일하게 작성할 것
  ...
  backend {
    ...
    group           = google_compute_region_instance_group_manager.web-mig.instance_group
		...
		# CONNECTION: 백엔드에서 처리할 수 있는 동시 연결 수를 기준으로 부하가 분산되는 방식을 결정합니다.
		# RATE: 초당 최대 대상 요청(쿼리) 수(RPS, QPS)입니다. 모든 최대 백엔드가 용량에 도달하거나 용량을 초과할 경우 대상 최대 RPS/QPS를 초과할 수 있습니다.
		# UTILIZATION: 인스턴스 그룹의 인스턴스 사용률에 따라 부하가 분산되는 방식을 결정합니다.
    balancing_mode  = "UTILIZATION"
		# 밸런싱 모드에 따라 비율을 설정함. [0.0 ~ 1.0]
    capacity_scaler = 1.0
  }
}
...
# USE_FIXED_PORT: 포트의 포트 번호는 상태 확인에 사용됩니다.
# USE_NAMED_PORT: portName은 상태 확인에 사용됩니다.
# USE_SERVING_PORT: NetworkEndpointGroup의 경우 각 네트워크 끝점에 대해 지정된 포트가 상태 확인에 사용됩니다.
#                   다른 백엔드의 경우 백엔드 서비스에 지정된 포트 또는 명명된 포트가 상태 확인에 사용됩니다. 
# 지정하지 않으면 HTTP 상태 확인은 port 및 portName 필드에 지정된 동작을 따릅니다.
http_health_check {
  port_specification = "USE_SERVING_PORT"
}
...
# UrlMaps는 수신 URL의 호스트 및 경로에 대해 정의한 규칙을 기반으로 요청을 백엔드 서비스로 라우팅하는 데 사용됩니다.
# 얘 이름이 LB 이름으로 설정됨
# 현재 호스트 & 경로에 대한 규칙이 없기 때문에 생략하였음
# 요청 헤더, 호스트, 경로 관리 등을 설정할 수 있음
